...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'verify_location') {
    ...
    try {
        ...
        $restaurant_lat = 14.7437232;
        $restaurant_lng =  -17.4674814;
        $allowed_radius = 150; 
    ...
    try {
        ...
        if ($choice == 'oui') {
            ...
            if ($stmt->rowCount() == 0) {
                ...
                $stmt = $conn->prepare("INSERT INTO newsletter (email, first_name, source, ip_address, date_inscription) VALUES (?, ?, 'commande', ?, NOW())");
...
$emailTemplate .= "
                ...
                // Pour paiement en ligne: déjà redirigé (voir ligne 293)
                // Pour paiement sur place: afficher le modal newsletter si email fourni
                if (!$needsOnlinePayment) {
                    if (!empty($email)) {
...
function setupNewsletterModal() {
    ...
    if (!email || !commandeId) {
        console.error('Newsletter: données manquantes');
        return;
    }
